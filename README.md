 ...
 * Decompose by **domain driven design**, by dividing the domain into subdomains like Order Taking Subdomain, Order Delivery subdomain, Order Accounting subdomain 
...
##### Transaction Outbox
* A service that uses a relational database uses **a single local transaction** to atomically write business data to the main table and insert the corresponding event into the outbox table.
...
##### REST gets a new verb 'QUERY'
* POST was always kinda designed semantically for "create something or take some action with my data" requests, but when we have a complex GET request, we cannot send the data we want to query using GET body, so we had to use something like
```
POST /orders/query
...
* In the future we can replace this POST with QUERY /orders request. The advantage being POST cannot be cached, whereas QUERY can be
* Massive GET URLs risk getting cut off by character limits, look incredibly messy, and often leak sensitive filter data (like user emails or search terms) into server access logs. 
* QUERY keeps this safely tucked inside the payload body.
